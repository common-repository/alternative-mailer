...
function & altmailer_create_multi_connections()
{
	...
	if( true === $altMailerOptions['use_sendmail'] )
	{
		...
		{
			$multi->addConnection( new Swift_Connection_Sendmail(SWIFT_SENDMAIL_AUTO_DETECT) );
		}
	}
...
function wp_mail($to, $subject, $message, $headers = '' )
{
	...
	if( count($headers) > 0 )
	{
		$objMessageHeaders =& $objMessage->headers;
		foreach( $headers as $name => $value)
		{
			$objMessageHeaders->set($name, $value);
		}
		$objMessage->headers =& $objMessageHeaders;
		unset($objMessageHeaders);
	}
